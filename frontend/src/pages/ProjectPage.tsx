  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', fontFamily: "'DM Sans', sans-serif", backgroundColor: '#fafaf8' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0097A7 0%, #00696F 100%)', padding: '28px 32px 56px', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
          ...
          <div style={{ position: 'relative', flex: 1, maxWidth: '280px', minWidth: '160px' }}>
            <Search size={14} aria-hidden="true" style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: 'rgba(255,255,255,0.6)', pointerEvents: 'none' }} />
            <input
              type="search"
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
            <button
              ...
              aria-label={showForm ? 'Cancel new project' : 'Create new project'}
            ...
            <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'rgba(255,255,255,0.25)', border: '2px solid rgba(255,255,255,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 600, color: '#fff' }}>
              AX
            </div>
          </div>
        </div>
      ...
      <div style={{ padding: '0 32px 32px', marginTop: '-36px' }}>
        ...
        {/* Search result count */}
        {search && (
          <div role="status" aria-live="polite" style={{ fontSize: '13px', color: '#6B7280', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Search size={13} aria-hidden="true" />
            <span>{filteredProjects.length} result{filteredProjects.length !== 1 ? 's' : ''} for "<strong>{search}</strong>"</span>
            <button onClick={() => setSearch('')} aria-label="Clear search" style={{ marginLeft: '4px', fontSize: '11px', color: '#0097A7', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", fontWeight: 600 }}>
              Clear
            </button>
          </div>
        )}
        ...
        {/* Error */}
        {error && (
          <div role="alert" style={{ background: '#FEF0F0', border: '1px solid #F7C1C1', borderRadius: '10px', padding: '10px 14px', fontSize: '13px', color: '#A32D2D', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
        ...
        {/* Create form */}
        {showForm && (
          <div style={{ background: '#ffffff', border: '1px solid #80DEEA', borderRadius: '16px', padding: '24px', boxShadow: '0 4px 24px rgba(0,151,167,0.12)', marginBottom: '24px' }}>
            <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: '20px', color: '#1C1C1A', marginBottom: '20px', fontWeight: 400 }}>
              Create new project
            </h2>

            {/* Color picker */}
            <div style={{ marginBottom: '18px' }}>
              <label style={{ fontSize: '11px', color: '#6B7280', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '10px', display: 'block' }}>
                Card color <span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(optional — leave for auto)</span>
              </label>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }} role="radiogroup" aria-label="Select card color">
                {CARD_COLORS.map((c, i) => (
                  <button
                    key={i}
                    role="radio"
                    aria-checked={selectedColor === i}
                    aria-label={i === 0 ? 'Auto color' : `Color option ${i}`}
                    onClick={() => setSelectedColor(i)}
                    style={{
                      width: '30px', height: '30px', borderRadius: '50%',
                      backgroundColor: i === 0 ? '#fff' : c.dot,
                      border: `2.5px solid ${selectedColor === i ? '#1C1C1A' : i === 0 ? '#D3D1C7' : c.dot}`,
                      cursor: 'pointer', transition: 'all .15s',
                      outline: selectedColor === i ? `3px solid ${i === 0 ? '#B8B7B0' : c.dot}55` : 'none',
                      outlineOffset: '3px',
                      display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                    }}
                  >
                    {i === 0 && (
                      <span style={{ fontSize: '10px', color: '#B8B7B0', fontWeight: 700 }}>A</span>
                    )}
                  </button>
                ))}
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '18px' }}>
              <div>
                <label htmlFor="project-name" style={{ fontSize: '11px', color: '#6B7280', display: 'block', marginBottom: '6px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                  Project name *
                </label>
                <input
                  id="project-name"
                  ...
                  style={{ ...inp, borderColor: error && !formData.name.trim() ? '#E24B4A' : '#E0DFD8' }}
              ...
              <div>
                <label htmlFor="project-desc" style={{ fontSize: '11px', color: '#6B7280', display: 'block', marginBottom: '6px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                  Description
                </label>
                <input
                  id="project-desc"
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button
                ...
                disabled={submitting}
                style={{ backgroundColor: submitting ? '#9CA3AF' : '#0097A7', color: '#fff', border: 'none', borderRadius: '8px', padding: '10px 22px', fontSize: '13px', fontWeight: 600, cursor: submitting ? 'not-allowed' : 'pointer', fontFamily: "'DM Sans', sans-serif" }}
              >
        ...
        {!loading && projects.length === 0 && (
          <div style={{ textAlign: 'center', padding: '72px 24px', background: '#fff', borderRadius: '16px', border: '1px solid rgba(0,0,0,0.07)' }}>
            <div style={{ width: '56px', height: '56px', background: '#E0F7FA', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', fontSize: '26px' }}>📋</div>
            <p style={{ fontFamily: "'DM Serif Display', serif", fontSize: '20px', color: '#1C1C1A', marginBottom: '8px' }}>No projects yet</p>
            <p style={{ fontSize: '13px', color: '#9CA3AF', marginBottom: '20px' }}>Click "New Project" to get started</p>
        ...
        {!loading && projects.length > 0 && filteredProjects.length === 0 && (
          <div style={{ textAlign: 'center', padding: '48px', background: '#fff', borderRadius: '16px', border: '1px solid rgba(0,0,0,0.07)' }}>
            <Search size={36} color="#D1D5DB" style={{ marginBottom: '14px' }} />
            <p style={{ fontFamily: "'DM Serif Display', serif", fontSize: '18px', color: '#1C1C1A', marginBottom: '8px' }}>No projects found</p>
            <p style={{ fontSize: '13px', color: '#9CA3AF', marginBottom: '16px' }}>No results for "{search}"</p>
          ...
          <div style={{ columns: '2', columnGap: '16px' }}>
            {filteredProjects.map((project) => {
              ...
              return (
                <div
                  key={project.id}
                  style={{ background: c.bg, border: `1.5px solid ${c.border}`, borderRadius: '16px', overflow: 'hidden', marginBottom: '16px', breakInside: 'avoid', cursor: 'pointer', transition: 'transform .15s, box-shadow .15s' }}
                ...
                >
                  <div style={{ padding: '16px 18px 12px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: 1, minWidth: 0 }}>
                        ...
                        <span style={{ fontSize: '15px', fontWeight: 700, color: c.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: "'DM Serif Display', serif" }}>
                          {project.name}
                        </span>
                      </div>
                      <button
                        ...
                        style={{ background: 'none', border: 'none', color: `${c.dot}88`, cursor: 'pointer', padding: '4px', display: 'flex', flexShrink: 0, marginLeft: '8px', borderRadius: '6px' }}
                        ...
                        onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.color = `${c.dot}88`; (e.currentTarget as HTMLButtonElement).style.background = 'none' }}
                      >
                        <Trash2 size={13} />
                      </button>
                    </div>
                    ...
                    {project.description && (
                      <div style={{ fontSize: '12px', color: `${c.text}99`, marginBottom: '12px', lineHeight: 1.6 }}>{project.description}</div>
                    )}

                    <div style={{ marginBottom: '5px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: '10px', color: `${c.text}88`, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                    ...
                    {tasks.length === 0 ? (
                      ...
                      <div key={task.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '5px 0', borderBottom: ti < tasks.length - 1 ? `1px solid ${c.dot}15` : 'none' }}>
                        ...
                        {!task.is_overdue && task.priority === 'High' && task.status !== 'Completed' && (
                          <span style={{ fontSize: '9px', fontWeight: 700, padding: '1px 7px', borderRadius: '20px', background: '#FEF0F0', color: '#A32D2D', whiteSpace: 'nowrap' }}>High</span>
                        )}
                      </div>
                    ))}
                  ...
                  <div style={{ padding: '8px 18px 14px', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                    ...
                    {project.overdue_tasks > 0 && (
                      <span style={{ fontSize: '10px', fontWeight: 600, padding: '3px 10px', borderRadius: '20px', background: '#FEF0F0', color: '#A32D2D', border: '1px solid #F7C1C1' }}>{project.overdue_tasks} overdue</span>
                    )}
                    {pct === 100 && (
                      <span style={{ fontSize: '10px', fontWeight: 600, padding: '3px 10px', borderRadius: '20px', background: '#E8F5F0', color: '#085041', border: '1px solid #9FE1CB' }}>✓ Complete</span>
                    )}
                  </div>
                </div>
              )
